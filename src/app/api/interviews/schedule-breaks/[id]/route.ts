  ...
  return withTenant(request, async (ctx) => {
    try {
      ...
      const existing = await ctx.run((tx) =>
        tx.interviewScheduleBreak.findFirst({ where: ctx.where({ id }), select: { id: true } }),
      );
      if (!existing) return NextResponse.json({ error: 'Break not found.' }, { status: 404 });
      const b = await ctx.run((tx) =>
        tx.interviewScheduleBreak.update({
          where: { id },
          data,
        }),
      );
...
export async function DELETE(
  ...
  return withTenant(_request, async (ctx) => {
    try {
      ...
      if (!existing) return NextResponse.json({ error: 'Break not found.' }, { status: 404 });
      await ctx.run((tx) => tx.interviewScheduleBreak.delete({ where: { id } }));
