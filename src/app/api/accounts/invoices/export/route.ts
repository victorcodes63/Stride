...
export async function GET(request: NextRequest) {
  return withAccountsTenant(request, async (ctx) => {
    ...
    try {
      const { invoices, subtotalByInvoice } = await ctx.run(async (tx) => {
        const rows = await tx.accountsInvoice.findMany({
          where: ctx.where(clientId ? { clientId } : {}),
          select: {
            id: true,
            invoiceNumber: true,
            clientId: true,
        ...
        for (const row of aggregates) {
          subtotals.set(row.invoiceId, Number(row._sum.amountExVat ?? 0));
        }

        return { invoices: rows, subtotalByInvoice: subtotals };
      });
