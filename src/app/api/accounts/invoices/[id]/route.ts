...
export async function GET(
  ...
  return withAccountsTenant(request, async (ctx) => {
    ...
    try {
      const inv = await ctx.run((tx) =>
        tx.accountsInvoice.findFirst({
          where: ctx.where({ id }),
          include: {
      ...
      const creditNoteRows = await ctx.run((tx) =>
        tx.accountsCreditNote.findMany({
          where: ctx.where({ originalInvoiceId: id }),
          select: {
            id: true,
      ...
      const creditTotalApplied = await ctx.run((tx) => sumCreditTotalForInvoice(tx, id));
