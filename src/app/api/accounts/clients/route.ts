...
export async function GET(request: NextRequest) {
  return withAccountsTenant(request, async (ctx) => {
    try {
      ...
      if (runSync) {
        syncResult = await ctx.run(async (tx) =>
          syncLinkedBillingClients(tx as unknown as PrismaClient),
        );
      }
...
export async function POST(request: NextRequest) {
  return withAccountsTenant(request, async (ctx) => {
    ...
    if (type === 'custom') {
      ...
      const rc = await ctx.run((tx) =>
        tx.client.findUnique({ where: { id: recruitmentClientId } }),
      );
      if (!rc) {
        return NextResponse.json({ error: 'Recruitment client not found.' }, { status: 404 });
      }
      const taken = await ctx.run((tx) =>
        tx.accountsClient.findUnique({ where: { recruitmentClientId } }),
      );
      if (taken) {
        return NextResponse.json(
      ...
      const oc = await ctx.run((tx) =>
        tx.outsourcingClient.findUnique({ where: { id: outsourcingClientId } }),
      );
      if (!oc) {
      ...
      const taken = await ctx.run((tx) =>
        tx.accountsClient.findUnique({ where: { outsourcingClientId } }),
      );
      if (taken) {
        return NextResponse.json(
