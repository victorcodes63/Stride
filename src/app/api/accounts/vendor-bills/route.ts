...
export async function GET(request: NextRequest) {
  return withAccountsTenant(request, async (ctx) => {
    ...
    try {
      const { bills, subtotalByBill } = await ctx.run(async (tx) => {
        const rows = await tx.accountsVendorBill.findMany({
          where: ctx.where(vendorId ? { vendorId } : {}),
          select: {
            id: true,
            vendorId: true,
            billRef: true,
        ...
        const aggregates =
          ids.length === 0
            ? []
            : await tx.accountsVendorBillLine.groupBy({
                by: ['billId'],
                where: { billId: { in: ids } },
        ...
        for (const row of aggregates) {
          subtotals.set(row.billId, Number(row._sum.amountExVat ?? 0));
        }

        return { bills: rows, subtotalByBill: subtotals };
      });
...
export async function POST(request: NextRequest) {
  return withAccountsTenant(request, async (ctx) => {
    ...
    try {
      const bill = await ctx.run(async (tx) => {
