...
export async function GET(request: NextRequest) {
  return withAccountsTenant(request, async (ctx) => {
    ...
    const [disbursementLines, clientPayments] = await ctx.run(async (tx) => {
      const lines = await tx.payrollDisbursementLine.findMany({
        where: {
      ...
      const payments = await tx.accountsClientPayment.findMany({
        where: {
          organizationId: ctx.organizationId,
          receivedAt: {
      ...
      return [lines, payments] as const;
    });
