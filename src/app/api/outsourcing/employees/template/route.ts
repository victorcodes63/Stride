  ...
  return withTenant(request, async (ctx) => {
    ...
    try {
      ...
      const { client, clientEmployees } = await ctx.run(async (tx) => {
        ...
        let resolvedClient: { id: string; name: string; departments: { name: string }[] } | null = null;
        let resolvedEmployees: Array<{
        ...
        return { client: resolvedClient, clientEmployees: resolvedEmployees };
      });
      ...
      if (isPayrollTemplate) {
        ...
        const deptList =
          client && client.departments.length > 0
        ...
        const clientInfo = client
          ? `Import employees for: ${client.name}. `
          : 'Import employees. Fill rows below, then select a client and use Import. ';
        sheet.addRow([
