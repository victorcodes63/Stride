...
export async function POST(request: NextRequest) {
  return withTenant(request, async (ctx) => {
    try {
      ...
      if (emailLower) {
        const dup = await ctx.run((tx) =>
          tx.employee.findFirst({
            where: ctx.where({ outsourcingClientId: clientId, email: emailLower }),
          }),
        );
        if (dup) {
          return NextResponse.json(
      ...
      if (idNumberNorm) {
        const idDup = await ctx.run((tx) =>
          tx.employee.findFirst({ where: ctx.where({ idNumber: idNumberNorm }) }),
        );
        if (idDup) {
          return NextResponse.json(
      ...
      if (attendancePolicyId) {
        await ctx
          .run((tx) =>
            tx.attendancePolicyAssignment.create({
              data: {
                organizationId: ctx.organizationId,
      ...
      if (leavePolicyId) {
        await ctx
          .run((tx) =>
            tx.leavePolicyAssignment.create({
              data: {
                organizationId: ctx.organizationId,
                employeeId: employee.id,
                leavePolicyId,
