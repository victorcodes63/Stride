...
import { jobLocationMatchesEntity, resolveEntityIdOrDefault } from '@/lib/entity-request';
  ...
  return withTenant(request, async (ctx) => {
    ...
    try {
      if (process.env.DATABASE_URL) {
        ...
        const [jobs, candidates, applications] = await ctx.run((tx) =>
          Promise.all([
            tx.job.findMany({
              where: ctx.where({
                AND: [
