...
export async function GET(request: NextRequest) {
  return withTenant(request, async (ctx) => {
    ...
    const overview = await ctx.run(async (tx) => {
      await syncStaffLeaveUsedDaysForUsersYear(tx, memberIds, year);
      ...
      const [members, pendingApprovals, approvedApps, upcoming, recentActions, annualType] = await Promise.all([
        tx.user.findMany({
        ...
        tx.leaveApprovalAction.findMany({
          where: {
            application: ctx.where({ userId: { in: memberIds } }),
    ...
    return NextResponse.json({
      year,
      kpis: {
        activeStaff: overview.members.length,
        pendingApprovals: overview.pendingApprovals,
        daysTakenYtd: overview.daysTakenYtd,
        onLeaveToday: overview.onLeaveToday,
        lowBalanceCount: overview.lowBalanceCount,
      },
      staff: overview.staff,
      upcoming: overview.upcoming.map((u) => ({
        id: u.id,
      ...
      recentActivity: overview.recentActions.map((a) => ({
        id: a.id,
        action: a.action,
