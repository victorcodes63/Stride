      ...
      if (includeHistory) {
        const history = await ctx.run((tx) =>
          tx.notificationDelivery.findMany({
            where: ctx.where({ recipientUserId: ctx.staff.id }),
            orderBy: { createdAt: 'desc' },
            take: limit,
            select: {
              id: true,
...
export async function PATCH(request: NextRequest) {
  return withTenant(request, async (ctx) => {
    ...
    try {
      if (body.action === 'run_escalation_sweep') {
        if (ctx.staff.role !== 'admin' && ctx.staff.staffUserType !== 'business_manager') {
          return NextResponse.json(
            { error: 'Only admins and business managers can run escalation sweeps.' },
            { status: 403 },
          );
        }
      ...
      await ctx.run((tx) =>
        tx.staffNotification.updateMany({
          where: ctx.where({ userId: ctx.staff.id, id: { in: ids } }),
          data: { readAt: now },
        }),
      );
