  ...
  try {
    ...
    const [projects, openTasks, overdueTasks, milestoneCounts, tasksDueSoon] = await Promise.all([
      ...
      prisma.projectTask.count({
        where: { project: { outsourcingClientId: clientId }, status: { not: 'done' } },
      }),
      prisma.projectTask.count({
        where: {
      ...
      prisma.projectMilestone.groupBy({
        by: ['status'],
        where: { project: { outsourcingClientId: clientId } },
        _count: { _all: true },
      }),
    ...
    const milestoneByProject = await prisma.projectMilestone.groupBy({
      by: ['projectId', 'status'],
