...
export default function NewCreditNotePage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/accounts/payment-accounts?activeOnly=1')
      ...
      .then((payload) => {
        ...
        setPaymentAccountId(
          (current) => current || accounts.find((a) => a.isDefault)?.id || accounts[0]?.id || '',
        );
      })
  ...
  const previewDraft = useMemo(() => {
    if (!inv) return null;
    const draftLines: InvoiceLineDraft[] = lines.map((l) => ({
      item: l.item,
    ...
    return {
      ...
      notes,
      lines: draftLines,
      previewInvoiceNumber: 1,
      originalInvoiceNumber: inv.invoiceNumber,
    };
  }, [inv, issueDate, paymentAccountId, notes, lines]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const j = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(j.error || `Failed (${r.status})`);
      router.push(`/dashboard/accounts/invoices/${invoiceId}`);
    } catch (err) {
    ...
    return (
      <div className="max-w-xl space-y-4">
        ...
        <p className="text-neutral-700 text-sm">
          Nothing left to credit on this invoice (or you don&apos;t have permission). Remaining
          creditable: {displayMoney(inv.remainingCreditable, inv.currency)}
        </p>
        <Link
    ...
    <DashboardPage>
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-6 items-start">
          <div className="lg:col-span-4 space-y-6 dashboard-surface p-5 sm:p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setLines((p) => [...p, { item: '', amountExVat: '', description: '' }])
                  }
                ...
                >
                  <div className="md:col-span-6">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLines((p) =>
                          p.map((row, i) => (i === index ? { ...row, item: e.target.value } : row)),
                        )
                      }
                    />
                  </div>
                  <div className="md:col-span-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLines((p) =>
                          p.map((row, i) =>
                            i === index ? { ...row, amountExVat: e.target.value } : row,
                          ),
                        )
                      }
                    />
                  ...
                  <div className="md:col-span-1 flex items-end">
                    <button
                      type="button"
                      onClick={() =>
                        setLines((p) => (p.length <= 1 ? p : p.filter((_, i) => i !== index)))
                      }
              ...
              {totals && (
                <p className="text-sm text-neutral-700">
                  ...
                  <span className="font-semibold tabular-nums">
                    {displayMoney(totals.totalIncVat, inv.currency)}
                  </span>
                </p>
              )}
            </div>
