...
export function createPayrollDisbursementsListHandler({ scope }: PayrollHandlerOptions) {
...
export function createPayrollDisbursementsCreateHandler({ scope }: PayrollHandlerOptions) {
  return async function POST(request: NextRequest) {
    return withTenant(request, async (ctx) => {
      ...
      await ctx.audit({
        ...
        route: `POST ${payrollApiBase(scope)}/disbursements`,
        metadata: {
          month,
          year,
          clientId,
