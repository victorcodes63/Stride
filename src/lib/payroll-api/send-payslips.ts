...
import { sendPayslipEmail, resolveClientPayslipSender } from '@/lib/email';
...
export function createPayrollSendPayslipsHandler({ scope }: PayrollHandlerOptions) {
    ...
    return withTenant(request, async (ctx) => {
              ...
              select: {
                ...
                client: {
                  select: {
                    ...
                    payslipFromName: true,
                    payslipReplyTo: true,
                    payslipSenderMode: true,
                    payslipSenderLocalPart: true,
                    payslipSenderDomain: true,
                    payslipDomainStatus: true,
                  },
                },
      ...
      for (const p of toProcess) {
        ...
        const biweekly =
          ...
          p.period1Gross != null &&
          p.period2Gross != null;
        const sender = resolveClientPayslipSender(p.employee.client);
        ...
        if (result.sent) {
          ...
          const resultDiagnostics =
            'diagnostics' in result
              ...
              : undefined;
          if (resultDiagnostics && !diagnostics) {
      ...
      await ctx.audit({
        ...
        route: `POST ${payrollApiBase(scope)}/send-payslips`,
        metadata: {
          month,
          year,
          clientId,
