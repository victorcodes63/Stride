...
import { payrollApiBase, type PayrollHandlerOptions } from '@/lib/payroll-api/scope';

const ALLOWED_STATUS = new Set(['pending', 'prepared', 'submitted', 'paid', 'overdue'] as const);

export function createPayrollStatutoryItemPatchHandler({ scope }: PayrollHandlerOptions) {
    ...
    return withTenant(request, async (ctx) => {
      try {
        ...
        await ctx.audit({
          ...
          entityType: 'StatutoryReturnItem',
          entityId: itemId,
          route: `PATCH ${payrollApiBase(scope)}/statutory/items/[itemId]`,
