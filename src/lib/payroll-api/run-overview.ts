...
import { resolvePayrollClientId, type PayrollHandlerOptions } from './scope';

export function createPayrollRunOverviewHandler({ scope }: PayrollHandlerOptions) {
  return async function GET(request: NextRequest) {
    return withTenant(request, async (ctx) => {
      ...
      // Setup readiness — is the workforce ready to run payroll at all?
      const staffWithSalaryCount = employees.filter((e) => Number(e.baseSalary ?? 0) > 0).length;

      // Pay-method split for the period: employees with complete bank details are
      // paid via the bank CSV file; everyone else defaults to the M-Pesa B2C rail.
      const hasBankByEmployee = new Map(
        employees.map((e) => [
          e.id,
          Boolean((e.bankName ?? '').trim()) && Boolean((e.bankAccountNumber ?? '').trim()),
        ]),
      );
      let mpesaCount = 0;
      let mpesaNet = 0;
      ...
      for (const p of payrolls) {
        ...
        if (hasBankByEmployee.get(p.employeeId)) {
          bankCount += 1;
          bankNet += net;
        } else {
          mpesaCount += 1;
      ...
      return NextResponse.json({
        ...
        setup: {
          ...
          staffWithSalaryCount,
          staffMissingSalaryCount: Math.max(0, employees.length - staffWithSalaryCount),
        },
        payMethods: {
          mpesaCount,
          mpesaNet,
