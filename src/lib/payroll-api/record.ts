...
import { getPayrollStatutoryRates } from '@/lib/country-config';
...
import { payrollApiBase, payrollBasePath, type PayrollHandlerOptions } from './scope';
...
export function createPayrollRecordGetHandler(_options: PayrollHandlerOptions) {
  return async function GET(request: NextRequest, { params }: RouteContext) {
    return withTenant(request, async (ctx) => {
      ...
      const attendanceSummary = await ctx.run((tx) =>
      ...
      const overtimeEligible = await ctx.run((tx) =>
        tx.attendanceDaySummary.aggregate({
          where: {
...
export function createPayrollRecordPatchHandler({ scope }: PayrollHandlerOptions) {
    ...
    return withTenant(request, async (ctx) => {
      try {
        ...
        await ctx.audit({
          ...
          entityType: 'Payroll',
          entityId: updated.id,
          route: `PATCH ${payrollApiBase(scope)}/[id]`,
          metadata: {
            month: updated.month,
            year: updated.year,
            ...
            const workflowRun = await ctx.run((tx) =>
              tx.workflowRun.findFirst({
                where: {
                  ...
                  entityId: `${updated.year}-${updated.month}-${existing.employee.outsourcingClientId}`,
                },
            ...
            await sendNotification({
              ...
              body:
                statusOverride === 'approved'
                  ? `${updated.month}/${updated.year} payroll has been approved by ${ctx.staff.name}.`
                  : `${updated.month}/${updated.year} payroll is now locked. Payslips can be distributed.`,
              href: payrollBasePath(scope),
              priority: 'info',
              channel: 'in_app',
              workflowRunId: workflowRun?.id,
              metadata: { month: updated.month, year: updated.year, approver: ctx.staff.name, workflowRunId: workflowRun?.id },
            });
          }
        } catch (err) {
