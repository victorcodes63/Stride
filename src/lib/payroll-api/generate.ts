...
import { getPayrollStatutoryRatesByClient, getPayrollStatutoryRates } from '@/lib/country-config';
...
import {
  ...
  type PayrollHandlerOptions,
} from './scope';

export function createPayrollGenerateHandler({ scope }: PayrollHandlerOptions) {
  return async function POST(request: NextRequest) {
    return withTenant(request, async (ctx) => {
      ...
      await ctx.audit({
        ...
        route: `POST ${payrollApiBase(scope)}/generate`,
        metadata: {
          month,
          year,
          clientId,
