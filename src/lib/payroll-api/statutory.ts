...
import { payrollApiBase, resolvePayrollClientId, type PayrollHandlerOptions } from '@/lib/payroll-api/scope';
...
const employeeScope = (clientId: string, organizationId: string) => ({
  outsourcingClientId: clientId,
  client: { organizationId },
});

export function createPayrollStatutoryGetHandler({ scope }: PayrollHandlerOptions) {
  return async function GET(request: NextRequest) {
    return withTenant(request, async (ctx) => {
      ...
      const empScope = employeeScope(clientId, ctx.organizationId);
...
export function createPayrollStatutoryPostHandler({ scope }: PayrollHandlerOptions) {
    ...
    return withTenant(request, async (ctx) => {
      try {
        ...
        const [employees, payrolls] = await Promise.all([
          ctx.run((tx) => tx.employee.findMany({ where: empScope, select: { id: true } })),
          ctx.run((tx) =>
            tx.payroll.findMany({
              where: { ...ctx.where(), month, year, employee: empScope },
              select: { id: true, grossPay: true, paye: true, nssf: true, nhif: true, ahl: true, deductions: true },
            }),
          ),
        ]);
        ...
        const saved = await ctx.run(async (tx) => {
        ...
        if (saved?.id) {
          ...
          await ctx.audit({
            ...
            entityType: 'StatutoryReturn',
            entityId: saved.id,
            route: `POST ${payrollApiBase(scope)}/statutory`,
