...
import { getDisbursementBatch, pollDisbursementBatch } from '@/lib/payroll-disbursement/service';
import { withTenant } from '@/lib/tenant-api';
import { payrollApiBase, type PayrollHandlerOptions } from './scope';

type RouteContext = { params: Promise<{ id: string }> };

export function createPayrollDisbursementDetailHandler(_options: PayrollHandlerOptions) {
...
export function createPayrollDisbursementPollHandler({ scope }: PayrollHandlerOptions) {
  return async function POST(request: NextRequest, context: RouteContext) {
    ...
    return withTenant(request, async (ctx) => {
      ...
      await ctx.audit({
        ...
        entityId: id,
        route: `POST ${payrollApiBase(scope)}/disbursements/[id]/poll`,
