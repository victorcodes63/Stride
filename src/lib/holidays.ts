...
type HolidayDb = Pick<Prisma.TransactionClient, 'publicHoliday'>;
...
const YEAR_CACHE = new Map<string, CachedYear>();
const YEAR_CACHE_TTL_MS = 5 * 60 * 1000;

function yearCacheKey(organizationId: string, year: number): string {
  return `${organizationId}:${year}`;
}
...
async function isPublicHolidayDirect(date: Date, db: HolidayDb): Promise<HolidayResult> {
  const day = date.getUTCDate();
  const month = date.getUTCMonth() + 1;
  const { start, end } = normalizeDateRange(date);

  const specific = await db.publicHoliday.findFirst({
    where: {
      isActive: true,
      recurring: false,
  ...
  const recurring = await db.publicHoliday.findFirst({
...
async function isPublicHolidayWithDb(date: Date, db: HolidayDb): Promise<HolidayResult> {
  const direct = await isPublicHolidayDirect(date, db);
  if (direct.isHoliday) return direct;

  if (date.getUTCDay() === 1) {
    const sunday = new Date(date);
    sunday.setUTCDate(sunday.getUTCDate() - 1);
    const sundayMatch = await isPublicHolidayDirect(sunday, db);
...
export async function isPublicHoliday(date: Date, organizationId: string): Promise<HolidayResult> {
  return withOrgContext(organizationId, (tx) => isPublicHolidayWithDb(date, tx));
}
...
async function loadHolidaysForYear(
  ...
  const specific = await db.publicHoliday.findMany({
    where: {
      isActive: true,
      recurring: false,
      date: {
  ...
  const recurring = await db.publicHoliday.findMany({
...
export async function getHolidaysForYear(
  ...
  const cacheKey = yearCacheKey(organizationId, year);
  ...
  const holidays = await withOrgContext(organizationId, (tx) => loadHolidaysForYear(year, tx));
  YEAR_CACHE.set(cacheKey, {
    loadedAt: Date.now(),
...
export function clearHolidayCache(organizationId?: string, year?: number): void {
  if (organizationId && typeof year === 'number') {
    YEAR_CACHE.delete(yearCacheKey(organizationId, year));
    return;
  }
  if (organizationId) {
    for (const key of YEAR_CACHE.keys()) {
      if (key.startsWith(`${organizationId}:`)) YEAR_CACHE.delete(key);
    }
    return;
  }
  YEAR_CACHE.clear();
...
export function createHolidayResolver(organizationId: string) {
  return (clockIn: Date | null, clockOut: Date | null) =>
    doesShiftTouchPublicHoliday(clockIn, clockOut, organizationId);
}
...
export async function doesShiftTouchPublicHoliday(
  ...
  return withOrgContext(organizationId, async (tx) => {
    ...
    while (probe.getTime() <= end.getTime()) {
      const holiday = await isPublicHolidayWithDb(probe, tx);
