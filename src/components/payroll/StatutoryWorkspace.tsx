...
export function StatutoryWorkspace({ config }: { config: PayrollSurfaceConfig }) {
  return (
    <Suspense fallback={<div className="h-40 animate-pulse rounded-2xl bg-neutral-100" />}>
      {config.mode === 'outsourcing' ? (
        <OutsourcingStatutoryWorkspace config={config} />
      ) : (
        <StatutoryWorkspaceInner config={config} client={INTERNAL_CLIENT_STATE} />
      )}
    </Suspense>
  );
}

function OutsourcingStatutoryWorkspace({ config }: { config: PayrollSurfaceConfig }) {
  const { clientId, clients, setClientId, showSwitcher, selectedClient } = useOutsourcingClient({ excludePrimary: true });
  return (
    <StatutoryWorkspaceInner
      config={config}
...
function StatutoryWorkspaceInner({
  config,
  client,
}: {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${config.apiBase}/statutory?${params.toString()}`);
  ...
  }, [month, year, clientId, config.apiBase]);

  useEffect(() => {
    if (isOutsourcing && !clientId) return;
    void fetchData();
  }, [fetchData, clientId, isOutsourcing]);
  ...
  const saveSnapshot = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`${config.apiBase}/statutory`, {
        method: 'POST',
  ...
  const updateItemStatus = async (itemId: string, status: ItemStatus) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${config.apiBase}/statutory/items/${itemId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
