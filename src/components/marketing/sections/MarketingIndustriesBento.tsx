...
export function MarketingIndustriesBento() {
  const live = INDUSTRY_VERTICALS.filter((v) => v.status === 'available');
  const featured = live.find((v) => v.id === 'logistics') ?? live[0];
  const rest = featured ? live.filter((v) => v.id !== featured.id) : [];
      ...
      <div className="mx-auto max-w-[1120px]">
        ...
        <div className="mt-[60px] grid gap-4 lg:grid-cols-3 lg:grid-rows-2">
          ...
          {rest.map((vertical, i) => (
            <MarketingReveal key={vertical.id} delay={0.12 + i * 0.06}>
              <article className="flex h-full min-h-[240px] flex-col justify-between rounded-[18px] border border-pub-border bg-white p-8 transition hover:-translate-y-1">
                <div>
                  <span className="inline-flex rounded-full bg-[var(--pub-primary)]/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--pub-primary)]">
                    Live
                  </span>
          ...
          {comingSoon.map((vertical, i) => (
            <MarketingReveal key={vertical.id} delay={0.12 + (rest.length + i) * 0.06}>
              <article className="flex h-full min-h-[240px] flex-col justify-between rounded-[18px] border border-pub-border bg-white p-8 transition hover:-translate-y-1">
                <div>
